...
| 方面 | 在 HTTP/3 上 |
| --- | --- |
| 声明了的请求 trailers | 与所有协议一样不转发：响应提交前返回 `501`；在 HTTP/3 上，之后则重置流。 |
| 上游响应 trailers | 与所有协议一样返回 `502`。 |
| `CONNECT` | Pingclair 不建立隧道。标准的 `CONNECT` 会被视为格式错误而重置，同时带有 `:scheme` 和 `:path` 的则返回 `501`。**下一版本**：返回带 `Allow` 的 `405`，与 HTTP/1.1 和 HTTP/2 上的响应相同。 |
