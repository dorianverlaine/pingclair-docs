...
`handle` ブロックはディレクティブを 1 つのルートにまとめます。兄弟関係にある `handle` ブロックは互いに排他的で、ちょうど 1 つだけが実行されます。マッチャーの無い `handle` はサイトのフォールバックになります。v0.2.0-rc.3 では、マッチしたパスが最も具体的なものが実行されます。**次のリリース：** 並べ替えで最初に来るものが実行されます。長いパスが短いパスより先で、それ以外はファイルに書いた順です（[どのルートが応答するか](#-どのルートが応答するか)を参照）。
