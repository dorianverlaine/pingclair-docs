...
DNS-01 は、ポート 80 で応答する代わりに TXT レコードを公開して、名前を管理していることを証明します。ワイルドカード証明書にはこの方式が必要で、ポート 80 を閉じているホストでも同じです。

⚠️ **v0.2.0-rc.3 では DNS-01 が完了しません。** このリリースは TXT レコードに誤った値を公開するため、どの注文も `Invalid` で終わります。修正と、以下で説明する単一のワイルドカード証明書は `main` にあり、まだリリースされていません。いま DNS-01 を使うには、インストーラの `--main` フラグで `main` を導入してください（[インストール](/ja/start/install/#-リリースバイナリからのインストール)）。この節の出力は、そのビルドでの動作です。

設定にはプロバイダーのブロックが必要です。
...
📌 **次のリリース。** 次のリリースでは、内部認証局を Caddy と同じくストア内の `pki/authorities/local/` に置き、リーフには中間証明書が署名します。旧来の `internal/` ディレクトリは移行されません。アップグレード後にサーバーは新しいルートを作成するため、すべてのクライアントで `pingclair trust` をもう一度実行して信頼し直す必要があります。
