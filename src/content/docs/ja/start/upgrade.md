...
スクリプトはリリースチャネルで最新リリースを探してそのタグを表示し、アーカイブの
SHA-256 を検証し、バイナリとユニットを置き換えてサービスを再起動します。
チャネルのホストに到達できないときは、代わりに GitHub の releases API に問い合わせます。
以下の実行はその経路をたどったため、`Fetching latest release` と表示しています。既存の
...
## ⚠️ 0.2.0 の前にアップグレードノートを読む

次のリリースでは、設定を変えなくても気づく挙動がいくつか変わります。どのルートがリクエストに応答するか、`encode` の無いサイトが圧縮するかどうか、リクエストボディの既定の上限、`remote_ip` が何にマッチするか、そして内部認証局がルートをどこに置くかです。[プロジェクト状況](/ja/project/status/#-次のリリースで変わること)に概要があり、[CHANGELOG](https://github.com/dorianverlaine/pingclair/blob/main/CHANGELOG.md) にはそれぞれのアップグレードノートがあります。新しいバイナリでサービスを再起動する前に、そのバイナリで設定を検証してください。
