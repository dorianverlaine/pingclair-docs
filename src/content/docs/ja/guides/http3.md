...
| 領域 | HTTP/3 での挙動 |
| --- | --- |
| 宣言されたリクエストトレーラー | 転送しません。レスポンスの確定前なら `501`、確定後ならストリームをリセットします。 |
| アップストリームのレスポンストレーラー | `502`。 |
| `CONNECT` | Pingclair はトンネルを開きません。**次のリリース：** HTTP/1.1 や HTTP/2 と同じく、`Allow` 付きの `405` を返します。 |
